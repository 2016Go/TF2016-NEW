...
static DHJson *Json_new (void) {
    return (DHJson*)calloc(1, sizeof(DHJson));
}

/* Delete a DHJson structure. */
void Json_dispose (DHJson *c) {
    DHJson *next;
    while (c) {
        next = c->next;
...
static const char* parse_number (DHJson *item, const char* num) {
...
static const char* parse_string (DHJson *item, const char* str) {
...
static const char* parse_value (DHJson *item, const char* value);
static const char* parse_array (DHJson *item, const char* value);
static const char* parse_object (DHJson *item, const char* value);
...
DHJson *Json_getItem (DHJson *object, const char* string) {
	DHJson *c = object->child;
	while (c && Json_strcasecmp(c->name, string))
		c = c->next;
	return c;
}

const char* Json_getString (DHJson* object, const char* name, const char* defaultValue) {
	object = Json_getItem(object, name);
	if (object) return object->valueString;
	return defaultValue;
}

float Json_getFloat (DHJson* value, const char* name, float defaultValue) {
	value = Json_getItem(value, name);
	return value ? value->valueFloat : defaultValue;
}

int Json_getInt (DHJson* value, const char* name, int defaultValue) {
	value = Json_getItem(value, name);
